...
import '../../../../community/chat/view/image_screen.dart';
import '../../../../community/chat/view/selected_media_view_screen.dart';
import '../../../../community/chat/view/video_view_screen.dart';
...
class _DatingChatViewState extends State<DatingChatView> {
  ...
  Future<void> fetchAllMessages() async {
    try {
      ...
      List response = await ServerClient.get(
        Urls.fetchAllMessagesSingleChat + widget.communityId,
      );
      if (response.first >= 200 && response.first < 300) {
        final model = GetPreviousChatModel.fromJson(response.last);
  ...
  @override
  void didUpdateWidget(DatingChatView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.communityId != widget.communityId ||
        oldWidget.userId != widget.userId) {
      // Reset state and re-initialize chat when widget is updated
      allMessages.clear();
      streamCtrl.add([]);
  ...
  disconnectFromSocket() {
    socket.disconnect();
    socket.disconnect();
    socket.off("message received singleChat");
    socket.off("connect");
  ...
  listenMessageEvent(VoidCallback setState) {
    socket.on("message received singleChat", (data) {
  ...
  sendMessage(String chatId, String sender, VoidCallback setState) async {
    ...
    webSocketSender("newMessageSingleChat", data);
    msgController.text = "";
    setState();
  }
